...
Page({
  ...
  onLoad: async function (options) {
    ...
    // 新增：接收用户选择的日期
    const selectedDate = options && options.selectedDate ? options.selectedDate : new Date().toISOString().split('T')[0];
        ...
        complete: () => {
          ...
          setTimeout(() => {
            ...
            if (pages.length > 1) {
              ...
              wx.navigateTo({
                url: `/pages/profile/baby-info?selectedFoods=${encodeURIComponent(JSON.stringify(selectedFoods))}`
              });
            }
          }, 1500);
        }
